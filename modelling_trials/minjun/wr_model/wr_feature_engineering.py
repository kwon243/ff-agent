"""
WR Feature Engineering - Adapted from QB/RB Pipeline
Create rolling averages, player history, and contextual features for WRs
"""
...
print("="*80)
print("WR FEATURE ENGINEERING")
print("="*80)
print("Adapting QB/RB pipeline for WR position...")

# Load combined WR/TE data and separate WRs
df = pd.read_csv('../../final_wr_and_te_data.csv')
print(f"\n✓ Loaded {len(df)} total WR/TE games from {df['YEAR'].min()}-{df['YEAR'].max()}")

# Separate WRs using inline percentage (TEs line up inline, WRs don't)
df['Position'] = (df['INL%'] > 15).map({True: 'TE', False: 'WR'})
df = df[df['Position'] == 'WR'].copy()

print(f"✓ Filtered to {len(df)} WR games")
print(f"✓ {df['PFR_ID'].nunique()} unique WRs")
...
def create_rolling_features(group):
    ...
    # Rolling receiving stats (WR-specific)
...
def create_career_features(group):
    ...
    # Career target metrics (WR-specific)
    ...
    # WR1/WR2/WR3 indicator (based on fantasy ranking)
    group['is_wr1'] = (group['POSITION_RANK'] <= 12).astype(int)
    group['is_wr2'] = ((group['POSITION_RANK'] > 12) & (group['POSITION_RANK'] <= 24)).astype(int)
...
# Age groups (WRs peak earlier than RBs)
...
# Alignment diversity (WRs who move around)
df['alignment_diversity'] = df['WID%'] + df['SLT%']  # TEs higher INL%, WRs higher WID+SLT
...
# Value over replacement (WR24 benchmark for WR2)
wr24_avg = df.groupby(['YEAR', 'Week'])['PPR'].apply(
    lambda x: x.nlargest(24).min() if len(x) >= 24 else x.median()
).reset_index(name='wr24_threshold')
df = df.merge(wr24_avg, on=['YEAR', 'Week'], how='left')
df['value_over_replacement'] = df['career_avg_ppr'] - df['wr24_threshold']

# Recent form indicator
df['recent_form'] = df['last_3_games_ppr'] - df['season_avg_ppr']

# Alpha WR indicator (high target share, WR1 on team)
df['is_alpha_wr'] = ((df['TGT'] > df['TGT'].quantile(0.75)) & (df['is_wr1'] == 1)).astype(int)

# Deep threat indicator (high ADOT)
df['is_deep_threat'] = (df['ADOT'] > 12).astype(int)
...
print(f"\n✓ Dataset: {len(df)} WR games")
print(f"✓ Players: {df['PFR_ID'].nunique()}")

# Save engineered dataset
df.to_csv('wr_data_engineered.csv', index=False)
print(f"\n✅ Saved: wr_data_engineered.csv")
...
new_feature_names = [
    ...
    'last_season_avg_ppr', 'is_wr1', 'is_wr2', 'team_win_rate', 'team_point_diff',
    ...
    'consistency_score', 'is_alpha_wr', 'is_deep_threat'
]
...
# Save importance
importance_df.to_csv('wr_feature_importance.csv', index=False)
