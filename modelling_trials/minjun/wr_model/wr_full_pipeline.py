"""
MINJUN'S FULL PIPELINE - WR PREDICTION MODEL
...
print("="*80)
print("MINJUN'S FULL PIPELINE - WR MODEL")
...
df = pd.read_csv('wr_data_engineered.csv')

# Load feature importance for selection
feat_imp = pd.read_csv('wr_feature_importance.csv')
...
with open('wr_xgb_final.pkl', 'wb') as f:
    pickle.dump(best_xgb, f)
print("✓ Saved: wr_xgb_final.pkl")

with open('wr_lgb_final.pkl', 'wb') as f:
    pickle.dump(best_lgb, f)
print("✓ Saved: wr_lgb_final.pkl")

with open('wr_cat_final.pkl', 'wb') as f:
    pickle.dump(best_cat, f)
print("✓ Saved: wr_cat_final.pkl")

# Save stacking ensemble
with open('wr_ensemble_model.pkl', 'wb') as f:
    pickle.dump(stacking_model, f)
print("✓ Saved: wr_ensemble_model.pkl")

# Save predictions
val_df['predicted_ppr'] = ensemble_val_preds
val_df[['PFR_ID', 'YEAR', 'Week', 'PPR', 'predicted_ppr']].to_csv('wr_ensemble_predictions.csv', index=False)
print("✓ Saved: wr_ensemble_predictions.csv")

# Save feature list
with open('wr_final_features.txt', 'w') as f:
    for feat in final_features:
        f.write(feat + '\n')
print("✓ Saved: wr_final_features.txt")
