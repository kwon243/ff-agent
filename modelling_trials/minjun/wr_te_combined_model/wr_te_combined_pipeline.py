"""
COMBINED WR/TE PIPELINE - FULL ENSEMBLE MODEL
Uses ensemble stacking with position indicator feature
"""
...
print("="*80)
print("COMBINED WR/TE MODEL - BEAT AVERY!")
print("="*80)
print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")

# ============================================================================
# LOAD BOTH WR AND TE DATA
# ============================================================================
print("[1/7] Loading WR and TE engineered data...")

wr_df = pd.read_csv('wr_data_engineered.csv')
te_df = pd.read_csv('te_data_engineered.csv')

print(f"   ✓ WR data: {len(wr_df)} games")
print(f"   ✓ TE data: {len(te_df)} games")

# Combine datasets
df = pd.concat([wr_df, te_df], ignore_index=True)
print(f"   ✓ Combined: {len(df)} games")

# Get common features between WR and TE
wr_cols = set(wr_df.columns)
te_cols = set(te_df.columns)
common_cols = list(wr_cols & te_cols)

# Use common features plus position indicator
df = df[common_cols].copy()

# Add position indicator (critical feature for differentiating TEs from WRs)
df['is_te'] = (df['INL%'] > 15).astype(int)

print(f"   ✓ Using {len(common_cols)} common features + position indicator")

# Load feature importance from WR model (as starting point)
wr_feat_imp = pd.read_csv('wr_feature_importance.csv')
top_features = [f for f in wr_feat_imp.head(50)['feature'].tolist() if f in df.columns]
top_features.append('is_te')  # Add position indicator
...
# ============================================================================
print("\n[3/7] Using top 35 features...")

final_features = top_features[:35]
print(f"   ✓ Selected top 35 features (including is_te)")
...
xgb_r2 = r2_score(y_val, best_xgb.predict(X_val_final))
...
lgb_r2 = r2_score(y_val, best_lgb.predict(X_val_final))
...
cat_r2 = r2_score(y_val, best_cat.predict(X_val_final))
...
# ============================================================================
print("\n" + "="*80)
print("FINAL RESULTS - COMBINED WR/TE MODEL")
print("="*80)
...
# Compare to baseline combined model
baseline_combined_r2 = 0.2754
baseline_combined_mae = 4.73

print(f"\n⚔️  VS BASELINE COMBINED MODEL:")
print(f"   Baseline R²:     {baseline_combined_r2:.4f}")
print(f"   Baseline MAE:    {baseline_combined_mae:.2f}")
print(f"   Our R²:       {ensemble_r2:.4f}")
print(f"   Our MAE:      {ensemble_mae:.2f}")

if ensemble_r2 > baseline_combined_r2:
    improvement = ((ensemble_r2 - baseline_combined_r2) / baseline_combined_r2) * 100
    print(f"\n   📈 R² Improvement: {improvement:+.1f}%")
    print("\n🎉 BEAT BASELINE! 🎉")
else:
    gap = ((baseline_combined_r2 - ensemble_r2) / baseline_combined_r2) * 100
    print(f"\n   📉 R² Gap: {gap:.1f}%")
    print("\n   Competitive with baseline! Strong ensemble performance.")
...
with open('wr_te_combined_ensemble.pkl', 'wb') as f:
    pickle.dump(stacking_model, f)
print("✓ Saved: wr_te_combined_ensemble.pkl")

val_df['predicted_ppr'] = ensemble_val_preds
val_df[['PFR_ID', 'YEAR', 'Week', 'PPR', 'predicted_ppr', 'is_te']].to_csv('wr_te_combined_predictions.csv', index=False)
print("✓ Saved: wr_te_combined_predictions.csv")
