...
top20_features = {
    # Core ranks & stability (from RF + Permutation + MI)
        ...
        # Rolling signals we can build from what’s present (see rolling function): REMOVED
}
#what to replace in either/or (1st over 2nd)
PAIR_PREFS = [("RUN_opponent","OFF_opponent")]
...
# 2b) save all per-model predictions (LONG format: one row per model-player-week)
all_preds_df.to_csv(f"rb_predictions_{TARGET_YEAR}_all_models_long.csv", index=False)
print(f"\nSaved rb_predictions_{TARGET_YEAR}_all_models_long.csv")

# 2c) build WIDE format: one row per player/week with a column per model
rb_wide = (
    all_preds_df
      .pivot_table(
...
# remove the column index name ('model') for cleanliness
rb_wide.columns.name = None

rb_wide.to_csv(f"rb_predictions_{TARGET_YEAR}_all_models_wide.csv", index=False)
print(f"Saved rb_predictions_{TARGET_YEAR}_all_models_wide.csv")
...
print(out.head(12).round(2).to_string(index=False))
out.to_csv(f"rb_fullpredictions_{TARGET_YEAR}_weekly.csv", index=False)
print(f"\nSaved rb_fullpredictions_{TARGET_YEAR}_weekly.csv")

# ============================================================
# SFrom Chat GPT: sTATIC SEASON-LONG MODELS FOR GYM (5-year weighted, pickled)
# ============================================================


print("\n=== Training static (season-long) RB models for gym use ===")
...
prefix = "rb"  # filenames: wrte_ridge.pkl, wrte_randomforest.pkl, etc.
...
print("\nFinished training + saving static RB models.")
